...
setwd('C:/Users/Joana Queiroz/Documents/Bioinformatics/ROS-ORF')
orf_df <- read.csv('orf_df.csv')

# Abbreviate ORF IDs in plot RosalindXXXX_ORF_X to ORF_X
orf_df$ID.abbrev <- sub(".*(orf.*)", "\\1", orf_df$ID)
...
orf_df <- orf_df[order(orf_df$First.bp), ]

# Create the plot with arrows
orf_plot_1 <- ggplot(orf_df) +
  geom_segment(
    aes(x = Start.in.DNA, xend = Stop.in.DNA, y = ID.abbrev, yend = ID.abbrev, color = Strand),
...
ggsave("orf_plot_1.pdf", plot = orf_plot_1)
...
orf_df_filter1 <- orf_df %>%
  ...
  theme(text = element_text(family="mono"),
    ...
    legend.position = c(0.8,0.8),
    legend.box.background = element_rect(color='black'))
ggsave("orf_plot_2.pdf", plot = orf_plot_2)